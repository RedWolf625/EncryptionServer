...
pthread_cond_t dec5 = PTHREAD_COND_INITIALIZER;   
...
void* decp_thread5(void* args) {
    pthread_mutex_lock(&mutex);
    if (pCount) {
        pthread_cond_wait(&dec5, &mutex);
    }
    dec_process();
...
int main(int argc, char* argv[]) {
    ...
    pthread_t dec_thread1, dec_thread2, dec_thread3, dec_thread4, dec_thread5;
    ...
    while (1) {
        ...
        if (pCount == 0) {
            ...
            pthread_create(&dec_thread1, NULL, decp_thread1, NULL);
            pthread_join(dec_thread1, NULL);
            pCount--;
        }
        if (pCount == 1) {
            pCount++;
            ...
            pthread_create(&dec_thread2, NULL, decp_thread2, NULL);
            pthread_join(dec_thread2, NULL);
            pCount--;
        }
        if (pCount == 2) {
            pCount++;
            ...
            pthread_create(&dec_thread3, NULL, decp_thread3, NULL);
            pthread_join(dec_thread3, NULL);
            pCount--;
        }
        if (pCount == 3) {
            pCount++;
            ...
            pthread_create(&dec_thread4, NULL, decp_thread4, NULL);
            pthread_join(dec_thread4, NULL);
            pCount--;
        }
        if (pCount == 4) {
            pCount++;
            printf("\nFIVE\n");
            // Now create the output thread
            pthread_create(&dec_thread5, NULL, decp_thread5, NULL);
            pthread_join(dec_thread5, NULL);
            pCount--;
        }
        else {}
